    ...
    internal class TextTokenizer
    {
        ...
        private void AdvanceToken(Boolean ignoreWhitespace)
        {
            ...
            while (finished != 0)
            {
                ...
                _nextTokenType = GetTokenType(activeCharacter);
                nextCharacterType = GetTokenType(nextCharacter);
                ...
                // this handles exponential values (e.g.1.0E10)
                if (_nextTokenType == TokenType.Number && (nextCharacter == 'E'||nextCharacter=='e'))
                {
                    nextCharacterType = TokenType.Number;
                    isNumber = true;
                }
                if (isNumber && (activeCharacter == 'E' || activeCharacter == 'e'))
                {
                    _nextTokenType = TokenType.Number;
                    if (nextCharacter == '-') nextCharacterType = TokenType.Number;
                }

                if (isNumber && activeCharacter == '-' && nextCharacterType == TokenType.Number)
                {
                    _nextTokenType = TokenType.Number;
                }
        ...
        private Char? GetChar(Int32 value)
        {
            ...
            else
            {
                ...
                if (_encoding.GetCharCount(_peekCharBytes, 0, 4) == _peekChar.Length)
                {
                ...
                {
                    c = _encoding.GetChars(_peekCharBytes, 0, 4)[0];
                }
            }
        ...
        private static TokenType GetTokenType(Char? character)
        {
            ...
            if (Char.IsWhiteSpace((Char)character) || Char.IsControl((Char)character))
            {
